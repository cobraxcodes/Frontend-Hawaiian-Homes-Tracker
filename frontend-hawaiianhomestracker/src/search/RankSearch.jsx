...
export default function RankSearch(){
  ...
  const fetchData = async(rank) =>{
    if(!rank){
      const res = await axios.get(`https://hawaiian-homes-tracker.onrender.com/applications?page=${page}&limit=20`)
      return res.data.applications
    }else{
      const res = await axios.get(`https://hawaiian-homes-tracker.onrender.com/applications/rank/${rank}`)
      return res.data.applications
    }
  }

      const {data,isLoading,isError,error}=useQuery({
    queryKey: ['rank',input, page],
    ...
    {isLoading && <p className="text-center">Finding Applications With Rankings of {input}</p>}
    {isError && (
